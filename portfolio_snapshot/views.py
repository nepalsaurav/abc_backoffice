...
def import_transactions(request):
    ...
    try:
        ...
        df = df.with_columns(
            pl.len()
            .over(["Parsed Date", "Stock", "Txn. Type", "Client Name"])
            .alias("txn_count")
        )
        df = df.with_columns((25.0 / pl.col("txn_count")).alias("dp_charge"))
        ...
        DailyTransaction.objects.bulk_create(transactions, ignore_conflicts=True)
        ...
        return JsonResponse(
            {
                "status": "error",
                "message": "Invalid Excel format or skip_rows mismatch.",
            },
            status=400,
        )
...
def sync_corporate_actions(request):
    ...
    from portfolio_snapshot.models import (
        DailyTransaction,
        CorporateAction,
        CorporateActionSync,
    )
    ...
    symbols = list(
        DailyTransaction.objects.annotate(upper_symbol=Upper("symbol"))
    ...
    try:
        ...
        CorporateAction.objects.bulk_create(actions_to_create, ignore_conflicts=True)
        CorporateActionSync.objects.create(status="Success")
        return JsonResponse(
            {"status": "success", "error": "successfully sync corporate actions"}
        )
    except Exception as e:
        CorporateActionSync.objects.create(status="Failed", error_log=str(e))
...
def corporate_action_dashboard(requests):
    ...
    sync_history = list(
        CorporateActionSync.objects.values(
            "id", "last_fetch_date", "status", "error_log"
...
def get_customer_list(request):
    clients = list(
        DailyTransaction.objects.values_list("client_name", flat=True).distinct()
    )
    return JsonResponse({"status": "success", "resp": clients})
